...
void * Get_User_Pointer(BLOCK_HEADER *current){
  return current + sizeof(BLOCK_HEADER);
}
...
void* Mem_Alloc(int size) {
    ...
    while(bh->packed_pointer != NULL){//curr
      ...
      printf("Enter loop\n");
      if(Is_Free(bh) == 1 && Get_Size(bh) <= size ){
      
        printf("Enter if \n");
        free_space = Get_Size(bh) - size;//reserve free space of size  bh - size allocated
        printf("%d\n", free_space);

        BLOCK_HEADER* new_bh = bh->packed_pointer-(free_space+sizeof(bh));//make new bh that is located in  final location  - the free space+size of bh 
        Set_Size(new_bh, free_space);//set the size of new bh to free space 
        Set_Size(bh, size);
        Set_Allocated(bh->packed_pointer);//set bh to be allocated
        new_bh->packed_pointer = end;//Get_Next_Header( ); 
        Set_Next_Pointer(bh,*new_bh);

       // Get_User_Pointer();
        break;
      }

     bh = Get_Next_Header(bh);
    }
    //while start at first header and 
    //find allocated space  while loop
    // allocate space
    // update block header, size, and pointer

    //create a new block header and update size and point to free space

    //search the blocks for fit 
    //no fit found add anoother block header 
